...
pub fn single_shooting_kernel_mpc_cost_hvp_states_u_seq(x0: &[f64], u_seq: &[f64], p: &[f64], v_u_seq: &[f64], hvp_u_seq: &mut [f64], x_traj: &mut [f64], work: &mut [f64]) {
    assert!(work.len() >= 42, "work is length {} but should be at least 42", work.len());
    assert_eq!(x0.len(), 2, "x0 is length {} but should be 2", x0.len());
    assert_eq!(u_seq.len(), 5, "u_seq is length {} but should be 5", u_seq.len());
    assert_eq!(p.len(), 2, "p is length {} but should be 2", p.len());
    assert_eq!(v_u_seq.len(), 5, "v_u_seq is length {} but should be 5", v_u_seq.len());
    assert_eq!(hvp_u_seq.len(), 5, "hvp_u_seq is length {} but should be 5", hvp_u_seq.len());
    assert_eq!(x_traj.len(), 12, "x_traj is length {} but should be 12", x_traj.len());
    let (state_history, rest) = work.split_at_mut(10);
...
fn single_shooting_kernel_mpc_cost_dynamics_jvp(x: &[f64], u: &[f64], p: &[f64], tangent_x: &[f64], tangent_u: &[f64], x_next: &mut [f64], work: &mut [f64]) {
    assert!(work.len() >= 3, "work is length {} but should be at least 3", work.len());
    assert_eq!(x.len(), 2, "x is length {} but should be 2", x.len());
    assert_eq!(u.len(), 1, "u is length {} but should be 1", u.len());
    assert_eq!(p.len(), 2, "p is length {} but should be 2", p.len());
    assert_eq!(tangent_x.len(), 2, "tangent_x is length {} but should be 2", tangent_x.len());
    assert_eq!(tangent_u.len(), 1, "tangent_u is length {} but should be 1", tangent_u.len());
    assert_eq!(x_next.len(), 2, "x_next is length {} but should be 2", x_next.len());
...
fn single_shooting_kernel_mpc_cost_dynamics_vjp_x_jvp(x: &[f64], u: &[f64], p: &[f64], cotangent_x_next: &[f64], tangent_x: &[f64], tangent_u: &[f64], tangent_cotangent_x_next: &[f64], vjp_x: &mut [f64], work: &mut [f64]) {
    assert!(work.len() >= 2, "work is length {} but should be at least 2", work.len());
    assert_eq!(x.len(), 2, "x is length {} but should be 2", x.len());
    assert_eq!(u.len(), 1, "u is length {} but should be 1", u.len());
    assert_eq!(p.len(), 2, "p is length {} but should be 2", p.len());
    ...
    assert_eq!(tangent_x.len(), 2, "tangent_x is length {} but should be 2", tangent_x.len());
    assert_eq!(tangent_u.len(), 1, "tangent_u is length {} but should be 1", tangent_u.len());
    assert_eq!(tangent_cotangent_x_next.len(), 2, "tangent_cotangent_x_next is length {} but should be 2", tangent_cotangent_x_next.len());
    assert_eq!(vjp_x.len(), 2, "vjp_x is length {} but should be 2", vjp_x.len());
...
#[allow(clippy::too_many_arguments)]
fn single_shooting_kernel_mpc_cost_dynamics_vjp_u_jvp(x: &[f64], u: &[f64], p: &[f64], cotangent_x_next: &[f64], tangent_x: &[f64], tangent_u: &[f64], tangent_cotangent_x_next: &[f64], vjp_u: &mut [f64], work: &mut [f64]) {
    assert!(!work.is_empty(), "work is length {} but should be at least 1", work.len());
    assert_eq!(x.len(), 2, "x is length {} but should be 2", x.len());
    assert_eq!(u.len(), 1, "u is length {} but should be 1", u.len());
    assert_eq!(p.len(), 2, "p is length {} but should be 2", p.len());
...
fn single_shooting_kernel_mpc_cost_stage_cost_grad_x_jvp(x: &[f64], u: &[f64], p: &[f64], tangent_x: &[f64], tangent_u: &[f64], ell: &mut [f64], work: &mut [f64]) {
    assert!(work.len() >= 2, "work is length {} but should be at least 2", work.len());
    assert_eq!(x.len(), 2, "x is length {} but should be 2", x.len());
    assert_eq!(u.len(), 1, "u is length {} but should be 1", u.len());
    assert_eq!(p.len(), 2, "p is length {} but should be 2", p.len());
...
fn single_shooting_kernel_mpc_cost_stage_cost_grad_u_jvp(x: &[f64], u: &[f64], p: &[f64], tangent_x: &[f64], tangent_u: &[f64], ell: &mut [f64], work: &mut [f64]) {
    assert!(!work.is_empty(), "work is length {} but should be at least 1", work.len());
    assert_eq!(x.len(), 2, "x is length {} but should be 2", x.len());
    assert_eq!(u.len(), 1, "u is length {} but should be 1", u.len());
    assert_eq!(p.len(), 2, "p is length {} but should be 2", p.len());
...
fn single_shooting_kernel_mpc_cost_terminal_cost_grad_x_jvp(x: &[f64], p: &[f64], tangent_x: &[f64], vf: &mut [f64], work: &mut [f64]) {
    assert!(!work.is_empty(), "work is length {} but should be at least 1", work.len());
    assert_eq!(x.len(), 2, "x is length {} but should be 2", x.len());
    assert_eq!(p.len(), 2, "p is length {} but should be 2", p.len());
    assert_eq!(tangent_x.len(), 2, "tangent_x is length {} but should be 2", tangent_x.len());
