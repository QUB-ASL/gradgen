...
pub fn single_shooting_kernel_mpc_cost_f_states(x0: &[f64], u_seq: &[f64], p: &[f64], cost: &mut [f64], x_traj: &mut [f64], work: &mut [f64]) -> Result<(), GradgenError> {
    if work.len() < 8 { return Err(GradgenError::WorkspaceTooSmall("work expected at least 8")); };
    if x0.len() != 2 { return Err(GradgenError::InputTooSmall("x0 expected length 2")); };
    if u_seq.len() != 5 { return Err(GradgenError::InputTooSmall("u_seq expected length 5")); };
    if p.len() != 2 { return Err(GradgenError::InputTooSmall("p expected length 2")); };
    if cost.len() != 1 { return Err(GradgenError::OutputTooSmall("cost expected length 1")); };
    if x_traj.len() != 12 { return Err(GradgenError::OutputTooSmall("x_traj expected length 12")); };
    let rest = work;
...
pub fn single_shooting_kernel_mpc_cost_grad_states_u_seq(x0: &[f64], u_seq: &[f64], p: &[f64], gradient_u_seq: &mut [f64], x_traj: &mut [f64], work: &mut [f64]) -> Result<(), GradgenError> {
    if work.len() < 14 { return Err(GradgenError::WorkspaceTooSmall("work expected at least 14")); };
    if x0.len() != 2 { return Err(GradgenError::InputTooSmall("x0 expected length 2")); };
    if u_seq.len() != 5 { return Err(GradgenError::InputTooSmall("u_seq expected length 5")); };
    if p.len() != 2 { return Err(GradgenError::InputTooSmall("p expected length 2")); };
    if gradient_u_seq.len() != 5 { return Err(GradgenError::OutputTooSmall("gradient_u_seq expected length 5")); };
    if x_traj.len() != 12 { return Err(GradgenError::OutputTooSmall("x_traj expected length 12")); };
    let rest = work;
    let (state_buffers, rest) = rest.split_at_mut(4);
    let (current_state_buf, next_state_buf) = state_buffers.split_at_mut(2);
...
fn single_shooting_kernel_mpc_cost_dynamics_vjp_x(_x: &[f64], _u: &[f64], p: &[f64], cotangent_x_next: &[f64], vjp_x: &mut [f64], work: &mut [f64]) {
...
fn single_shooting_kernel_mpc_cost_dynamics_vjp_u(_x: &[f64], _u: &[f64], p: &[f64], cotangent_x_next: &[f64], vjp_u: &mut [f64], work: &mut [f64]) {
    work[0] = cotangent_x_next[1] * p[1];
    work[0] += cotangent_x_next[0];
    vjp_u[0] = work[0];
}

fn single_shooting_kernel_mpc_cost_stage_cost_grad_x(x: &[f64], _u: &[f64], _p: &[f64], ell: &mut [f64], work: &mut [f64]) {
    work[0] = 2.0_f64 * x[0];
    work[1] = 4.0_f64 * x[1];
    ell[0] = work[0];
    ell[1] = work[1];
...
fn single_shooting_kernel_mpc_cost_stage_cost_grad_u(_x: &[f64], u: &[f64], p: &[f64], ell: &mut [f64], work: &mut [f64]) {
    work[0] = 0.6_f64 * u[0];
    work[0] += p[0];
    ell[0] = work[0];
}
...
pub fn single_shooting_kernel_mpc_cost_hvp_states_u_seq(x0: &[f64], u_seq: &[f64], p: &[f64], v_u_seq: &[f64], hvp_u_seq: &mut [f64], x_traj: &mut [f64], work: &mut [f64]) -> Result<(), GradgenError> {
    if work.len() < 32 { return Err(GradgenError::WorkspaceTooSmall("work expected at least 32")); };
    if x0.len() != 2 { return Err(GradgenError::InputTooSmall("x0 expected length 2")); };
    if u_seq.len() != 5 { return Err(GradgenError::InputTooSmall("u_seq expected length 5")); };
    if p.len() != 2 { return Err(GradgenError::InputTooSmall("p expected length 2")); };
    if v_u_seq.len() != 5 { return Err(GradgenError::InputTooSmall("v_u_seq expected length 5")); };
    if hvp_u_seq.len() != 5 { return Err(GradgenError::OutputTooSmall("hvp_u_seq expected length 5")); };
    if x_traj.len() != 12 { return Err(GradgenError::OutputTooSmall("x_traj expected length 12")); };
    let rest = work;
    ...
    for stage_index in 0..5 {
        ...
        single_shooting_kernel_mpc_cost_dynamics(current_state, u_t, p, next_state, stage_work);
        single_shooting_kernel_mpc_cost_dynamics_jvp(current_state, u_t, p, current_tangent, v_u_t, next_tangent, stage_work);
        state_history[(stage_index * 2)..((stage_index + 1) * 2)].copy_from_slice(next_state);
    ...
    single_shooting_kernel_mpc_cost_terminal_cost_grad_x(current_state, p, lambda_current, stage_work);
    single_shooting_kernel_mpc_cost_terminal_cost_grad_x_jvp(current_state, p, current_tangent, mu_current, stage_work);
    for stage_index in (1..5).rev() {
        let x_t = &state_history[((stage_index - 1) * 2)..(stage_index * 2)];
        let u_t = &u_seq[stage_index..(stage_index + 1)];
        ...
        single_shooting_kernel_mpc_cost_stage_cost_grad_u_jvp(x_t, u_t, p, tangent_x_t, v_u_t, hvp_u_t, stage_work);
        single_shooting_kernel_mpc_cost_dynamics_vjp_u_jvp(x_t, u_t, p, &lambda_current[..], tangent_x_t, v_u_t, &mu_current[..], temp_control, stage_work);
        hvp_u_t[0] += temp_control[0];
        single_shooting_kernel_mpc_cost_stage_cost_grad_x(x_t, u_t, p, lambda_next, stage_work);
        single_shooting_kernel_mpc_cost_dynamics_vjp_x(x_t, u_t, p, &lambda_current[..], temp_state, stage_work);
        lambda_next[0] += temp_state[0];
        lambda_next[1] += temp_state[1];
        single_shooting_kernel_mpc_cost_stage_cost_grad_x_jvp(x_t, u_t, p, tangent_x_t, v_u_t, mu_next, stage_work);
        single_shooting_kernel_mpc_cost_dynamics_vjp_x_jvp(x_t, u_t, p, &lambda_current[..], tangent_x_t, v_u_t, &mu_current[..], temp_state, stage_work);
    ...
    single_shooting_kernel_mpc_cost_stage_cost_grad_u_jvp(x0, u_t, p, next_tangent, v_u_t, hvp_u_t, stage_work);
    single_shooting_kernel_mpc_cost_dynamics_vjp_u_jvp(x0, u_t, p, &lambda_current[..], next_tangent, v_u_t, &mu_current[..], temp_control, stage_work);
    hvp_u_t[0] += temp_control[0];
    Ok(())
}

fn single_shooting_kernel_mpc_cost_dynamics_jvp(_x: &[f64], _u: &[f64], p: &[f64], tangent_x: &[f64], tangent_u: &[f64], x_next: &mut [f64], work: &mut [f64]) {
...
fn single_shooting_kernel_mpc_cost_dynamics_vjp_x_jvp(_x: &[f64], _u: &[f64], p: &[f64], _cotangent_x_next: &[f64], _tangent_x: &[f64], _tangent_u: &[f64], tangent_cotangent_x_next: &[f64], vjp_x: &mut [f64], work: &mut [f64]) {
...
#[allow(clippy::too_many_arguments)]
fn single_shooting_kernel_mpc_cost_dynamics_vjp_u_jvp(_x: &[f64], _u: &[f64], p: &[f64], _cotangent_x_next: &[f64], _tangent_x: &[f64], _tangent_u: &[f64], tangent_cotangent_x_next: &[f64], vjp_u: &mut [f64], work: &mut [f64]) {
    work[0] = p[1] * tangent_cotangent_x_next[1];
    work[0] += tangent_cotangent_x_next[0];
    vjp_u[0] = work[0];
}

fn single_shooting_kernel_mpc_cost_stage_cost_grad_x_jvp(_x: &[f64], _u: &[f64], _p: &[f64], tangent_x: &[f64], _tangent_u: &[f64], ell: &mut [f64], work: &mut [f64]) {
...
fn single_shooting_kernel_mpc_cost_stage_cost_grad_u_jvp(_x: &[f64], _u: &[f64], _p: &[f64], _tangent_x: &[f64], tangent_u: &[f64], ell: &mut [f64], work: &mut [f64]) {
    work[0] = 0.6_f64 * tangent_u[0];
    ell[0] = work[0];
}

fn single_shooting_kernel_mpc_cost_terminal_cost_grad_x_jvp(_x: &[f64], _p: &[f64], tangent_x: &[f64], vf: &mut [f64], work: &mut [f64]) {
...
pub fn single_shooting_kernel_mpc_cost_f_grad_states_u_seq(x0: &[f64], u_seq: &[f64], p: &[f64], cost: &mut [f64], gradient_u_seq: &mut [f64], x_traj: &mut [f64], work: &mut [f64]) -> Result<(), GradgenError> {
    if work.len() < 15 { return Err(GradgenError::WorkspaceTooSmall("work expected at least 15")); };
    if x0.len() != 2 { return Err(GradgenError::InputTooSmall("x0 expected length 2")); };
    if u_seq.len() != 5 { return Err(GradgenError::InputTooSmall("u_seq expected length 5")); };
    if p.len() != 2 { return Err(GradgenError::InputTooSmall("p expected length 2")); };
    if cost.len() != 1 { return Err(GradgenError::OutputTooSmall("cost expected length 1")); };
