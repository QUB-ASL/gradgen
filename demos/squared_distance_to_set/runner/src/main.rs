use squared_distance_to_set_kernel::*;
...
fn main() {
    let primal_metadata = squared_distance_to_set_kernel_distance_energy_f_meta();
    let gradient_metadata =
        squared_distance_to_set_kernel_distance_energy_grad_x_f_meta();
    ...
    squared_distance_to_set_kernel_distance_energy_f(&x, &mut y, &mut y_work)
        .unwrap();
    ...
    squared_distance_to_set_kernel_distance_energy_grad_x_f(
        &x,
        &mut grad,
        &mut grad_work,
    )
