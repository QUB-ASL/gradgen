...
- one Rust crate containing both the staged primal and staged gradient kernels
- loop-preserving code generation from the staged composed source

## Files

- [`main.py`](./main.py): builds the composed function and generates Rust
- `composed_kernel/`: generated by running `main.py`
- `runner/`: a small Rust binary crate that depends on `composed_kernel` by
  path, calls the staged primal and gradient kernels, and prints their outputs
...
```text
demos/composed_function/composed_kernel
```

To see how the generated crate can be used from Rust in practice, run:

```bash
cd demos/composed_function/runner
cargo run
```
