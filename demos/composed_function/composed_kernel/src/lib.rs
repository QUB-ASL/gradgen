...
pub fn composed_kernel_composed_demo_f(
    ...
    for repeat_index in 0..5 {
        composed_kernel_composed_demo_f_repeat_0_g(
            current_state,
            &parameters[0 + (repeat_index * 2)..0 + ((repeat_index + 1) * 2)],
            next_state,
            stage_work,
        );
...
fn composed_kernel_composed_demo_f_repeat_0_g(
    state: &[f64],
    p: &[f64],
    next_state: &mut [f64],
    work: &mut [f64],
...
fn composed_kernel_composed_demo_f_terminal_h(
    state: &[f64],
    pf: &[f64],
    y: &mut [f64],
    work: &mut [f64],
...
pub fn composed_kernel_composed_demo_grad_x(
    ...
    for repeat_index in 0..5 {
        ...
        let stage_end = stage_start + 2;
        {
            ...
            composed_kernel_composed_demo_grad_x_repeat_0_g(
                current_state,
                &parameters[0 + (repeat_index * 2)..0 + ((repeat_index + 1) * 2)],
                next_state,
                stage_work,
    ...
    composed_kernel_composed_demo_grad_x_terminal_h_grad(
        current_state,
        &parameters[10..11],
        lambda_a,
        stage_work,
    ...
    for repeat_index in (0..5).rev() {
        ...
        if stage_index == 0 {
            if current_lambda_is_a {
                composed_kernel_composed_demo_grad_x_repeat_0_g_vjp(
                    x,
                    &parameters[0 + (repeat_index * 2)..0 + ((repeat_index + 1) * 2)],
                    &lambda_a[..],
                    lambda_b,
...
fn composed_kernel_composed_demo_grad_x_repeat_0_g(
    state: &[f64],
    p: &[f64],
    next_state: &mut [f64],
    work: &mut [f64],
...
fn composed_kernel_composed_demo_grad_x_repeat_0_g_vjp(
    state: &[f64],
    p: &[f64],
...
fn composed_kernel_composed_demo_grad_x_terminal_h_grad(
    state: &[f64],
    pf: &[f64],
    y: &mut [f64],
    _work: &mut [f64],
