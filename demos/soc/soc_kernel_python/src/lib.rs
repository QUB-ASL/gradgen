...
use ::soc_kernel as gradgen_low_level;
...
fn all_functions_impl(py: Python<'_>) -> PyResult<Py<PyAny>> {
    Ok(PyList::new(py, ["energy", "energy_grad_x"])?
        .into_any()
        .unbind())
}
...
fn workspace_for_function_impl(py: Python<'_>, function_name: &str) -> PyResult<Py<Workspace>> {
    match function_name {
        ...
        "energy_grad_x" | "soc_kernel_energy_grad_x" => {
            let mut values = Vec::with_capacity(0);
            values.resize(0, 0.0_f64);
            Py::new(
                py,
                Workspace {
                    function_name: "energy_grad_x",
                    values,
                },
            )
        }
...
fn function_info_impl(py: Python<'_>, function_name: &str) -> PyResult<Py<PyAny>> {
    match function_name {
        "energy" | "soc_kernel_energy" => {
            let metadata = gradgen_low_level::soc_kernel_energy_meta();
            build_function_info(py, "energy", metadata)
        }
        "energy_grad_x" | "soc_kernel_energy_grad_x" => {
            let metadata = gradgen_low_level::soc_kernel_energy_grad_x_meta();
            build_function_info(py, "energy_grad_x", metadata)
        }
        _ => Err(PyValueError::new_err(format!(
            "unknown generated function {function_name:?}"
        ))),
...
fn call_impl(
    ...
    match function_name {
        "energy" | "soc_kernel_energy" => call_soc_kernel_energy_from_tuple_impl(py, inputs),
        "energy_grad_x" | "soc_kernel_energy_grad_x" => {
            call_soc_kernel_energy_grad_x_from_tuple_impl(py, inputs)
        }
        _ => Err(PyValueError::new_err(format!(
            "unknown generated function {function_name:?}"
        ))),
...
fn call_soc_kernel_energy_impl(
    py: Python<'_>,
    input_0: Vec<f64>,
    ...
    gradgen_low_level::soc_kernel_energy(
        &input_0[..],
        &mut output_0[..],
...
fn call_soc_kernel_energy_from_tuple_impl(
    py: Python<'_>,
    inputs: &Bound<'_, PyTuple>,
) -> PyResult<Py<PyAny>> {
    let metadata = gradgen_low_level::soc_kernel_energy_meta();
    ...
    let workspace = inputs.get_item(1)?.extract::<PyRefMut<'_, Workspace>>()?;
    call_soc_kernel_energy_impl(py, input_0, workspace)
}

#[pyfunction(name = "energy")]
fn py_soc_kernel_energy(
    py: Python<'_>,
    arg_0: &Bound<'_, PyAny>,
    workspace: PyRefMut<'_, Workspace>,
) -> PyResult<Py<PyAny>> {
    let input_0 = extract_values(arg_0, 3, "x")?;
    call_soc_kernel_energy_impl(py, input_0, workspace)
}

fn call_soc_kernel_energy_grad_x_impl(
    py: Python<'_>,
    input_0: Vec<f64>,
    mut workspace: PyRefMut<'_, Workspace>,
) -> PyResult<Py<PyAny>> {
    let metadata = gradgen_low_level::soc_kernel_energy_grad_x_meta();
    if workspace.function_name != "energy_grad_x"
        && workspace.function_name != metadata.function_name
    {
        return Err(PyValueError::new_err(format!(
            "workspace_for_function({:?}) must be used with {}",
            workspace.function_name, "energy_grad_x"
        )));
    }
    if workspace.values.len() < metadata.workspace_size {
        return Err(PyValueError::new_err(format!(
    ...
    let mut output_0 = [0.0_f64; 3];

    gradgen_low_level::soc_kernel_energy_grad_x(
        &input_0[..],
        &mut output_0[..],
        workspace.values.as_mut_slice(),
    )
...
fn call_soc_kernel_energy_grad_x_from_tuple_impl(
    py: Python<'_>,
    inputs: &Bound<'_, PyTuple>,
) -> PyResult<Py<PyAny>> {
    let metadata = gradgen_low_level::soc_kernel_energy_grad_x_meta();
    ...
    if inputs.len() != expected_arg_count {
        return Err(PyValueError::new_err(format!(
            "energy_grad_x expected {expected_arg_count} arguments including workspace"
        )));
    }
    ...
    let workspace = inputs.get_item(1)?.extract::<PyRefMut<'_, Workspace>>()?;
    call_soc_kernel_energy_grad_x_impl(py, input_0, workspace)
}

#[pyfunction(name = "energy_grad_x")]
fn py_soc_kernel_energy_grad_x(
    py: Python<'_>,
    arg_0: &Bound<'_, PyAny>,
    workspace: PyRefMut<'_, Workspace>,
) -> PyResult<Py<PyAny>> {
...
fn gradgen_python_interface(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(py_soc_kernel_energy, m)?)?;
    m.add_function(wrap_pyfunction!(py_soc_kernel_energy_grad_x, m)?)?;
