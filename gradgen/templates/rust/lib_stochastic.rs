...
pub fn total_cost_gradient_bw(
    ...
    for i in NODES_AT_STAGE_FROM[stage]..=NODES_AT_STAGE_TO[stage] {
        ...
        if NPRED > 1 {
            ws.a_at_node[i * NX..(i + 1) * NX].copy_from_slice(&ws.temp_nx_a);
...
mod tests {
    ...
    #[test]
    fn tst_it() {
        let mut workspace = super::BackwardGradientWorkspace::new();
        let x0 = vec![0.1; NX];
        ...
        for _ in 1..runs {
            super::total_cost_gradient_bw(&x0, &us, &mut grad_stoc, &mut workspace);
        }
        
        let new_now = Instant::now();
        println!("{:?}", new_now.duration_since(now)/runs);
        // println!("DVN = {:?}", grad_stoc);
    }
}