...
class TestCallbacksAdditional(unittest.TestCase):
    ...
    def test_invoke_custom_hvp_callback_signature_and_ordering(self):
        # When inspect.signature raises, the code should call
        # (value, tangent, w)
        ...
        with unittest.mock.patch.object(
                callbacks.inspect, 
                "signature", new=_raise_type):
            out = callbacks.invoke_custom_hvp_callback(
                cb_fallback, "x", "t", "w", 1)
    ...
    def test_coerce_symbolic_vector_and_matrix_stronger_checks(self):
        ...
        self.assertEqual(
            tuple(tuple(e.value for e in row) for row in mat), 
            ((1.0, 0.0), (0.0, 2.0)))
    ...
    def test_coerce_numeric_vector_handles_sxvector_and_item_edge(self):
        ...
        # object with item() returning non-scalar should raise via 
    ...
    def test_build_custom_hvp_from_hessian_numeric_evaluation(self):
        ...
        hvp_expr = callbacks._build_custom_hvp_from_hessian(
            spec_scalar, SX.const(1.0), SX.const(4.0), ())
        # hvp_expr should evaluate to 12.0 regardless of input, 
        # so create a dummy Function
        x = SX.sym("x")
        f = Function("f", [x], [hvp_expr])
        self.assertEqual(f(0.0), 12.0)

        # vector case: use simple diagonal Hessian [[1,0],[0,2]] 
        # and tangent [3,4]
        ...
        hvp_vec = callbacks._build_custom_hvp_from_hessian(
            spec_vec, tangent, tangent, ())
