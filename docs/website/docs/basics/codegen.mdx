...
sidebar_position: 4
slug: /basics/codegen
description: "Generate embeddable Rust code from symbolic Gradgen functions, including no_std kernels, workspace slices, and derivative bundles."
tags:
  - basics
  - rust
  - code-generation
...
The custom header is rendered as a small Jinja2 template before it is inserted
into `src/lib.rs`. The most useful built-in variables are `scalar_type`,
`backend_mode`, and `math_library`.
...
In this example, gradgen renders the header with `scalar_type = "f32"` before
writing it into the generated crate.
