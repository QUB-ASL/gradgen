...
sidebar_position: 6
slug: /basics/ocp
description: "Model and differentiate single-shooting optimal control problems in Gradgen, including stage costs, terminal costs, gradients, and Hessian-vector products."
tags:
  - basics
  - optimal-control
  - control
...
x_{k+1} = f(x_k, u_k, p),
```
where $x_k \in \mathbb{R}^{n_x}$ is the state,
...
$\nabla V_N(u_{{\rm seq}}, p)$ looks like
(excerpt from auto-generated code):
...
$\tilde{\ell}(x, u, p, c) = \ell(x, u, p) + \tfrac{c}{2}\Vert q(x, u, p) \Vert_2^2$
and the terminal cost function
$\tilde{V}_f(x, p, c) = V_f(x, p) + \tfrac{c}{2}\Vert q_N(x, p)\Vert_2^2$.

</details>

### Example

As an example, suppose we have the stage penalty function
$q:\mathbb{R}^2\times \mathbb{R} \times \mathbb{R}^2 \to \mathbb{R}^2$
given by
```math
q(x, u, p) = \begin{bmatrix} x_1 + u - p_1 \\ x_2 - p_2 \end{bmatrix},
```
and the terminal penalty function
$q_N:\mathbb{R}^2\times \mathbb{R}^2 \to \mathbb{R}$ given by
```math
...
[![Try it In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1rukXaRu4cUAbncgUhOFkNmVuF9wS_BeY?usp=sharing)
