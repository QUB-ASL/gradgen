...
$$V_N(x_0, u_\{\mathrm\{seq\}\}, p, c) =
\sum_\{t=0\}^\{N-1\} \ell(x_t, u_t, p)
+ \tfrac\{c\}\{2\}\Vert q(x_t, u_t, p) \Vert_2^2 + V_f(x_N, p)
+ \tfrac\{c\}\{2\}\Vert q_N(x_N, p)\Vert_2^2,$$
...
$\tilde\{\ell\}(x, u, p, c) = \ell(x, u, p)
+ \tfrac\{c\}\{2\}\Vert q(x, u, p) \Vert_2^2$
and the terminal cost function
$\tilde\{V\}_f(x, p, c) = V_f(x, p)
+ \tfrac\{c\}\{2\}\Vert q_N(x, p)\Vert_2^2$.
