---
sidebar_position: 1
slug: /basics/symbolic
description: "Create scalar and vector symbolic expressions in Gradgen, including slicing, constants, vector construction, and quadratic forms."
tags:
  - basics
  - symbolic
keywords:
...
For example, to define the function $f(x) = ux/\Vert x \Vert_{1}$ we can do  
...
0, & \text{ if } x < p 
\\
(x-p)^2, & \text{ if } x \geq p 
\end{cases}
```

This defines a function $f(x, p)$.
...
where $f_1$ and $f_2$ are expressions and the condition is an inequality
...
Note that the function $f$ may not be differentiable everywhere. Nevertheless, 
gradgen will always compute a derivative, provided $f_1$ and $f_2$ are differentiable.
...
We know that $f'(x) = 0$ for all $x\neq 0$, while at $x=0$ the derivative 
is not defined. However, at $x=0$ gradgen uses the branch $f(x) = 1$ and computes
the derivative; it is
