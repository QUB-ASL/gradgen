---
sidebar_position: 7
slug: /basics/high_order_functions
description: "Build batched map, zip, and reduce kernels in Gradgen to exploit structure and generate smaller Rust code."
tags:
  - basics
  - batching
...
$\mathbf{map}_{u, N}: \mathbf{x} \mapsto \mathbf{y}$.
...
```math
\mathbf{a} = (a^{(1)}, \dots, a^{(M)}), \qquad \mathbf{c} = (c^{(1)}, \dots, c^{(M)}),
```
...
```math
\mathbf{z} =
(g(a^{(1)}, c^{(1)}), \dots, g(a^{(M)}, c^{(M)})).
```

In other words, it is the mapping
$\mathbf{zip}_{b, M}: (\mathbf{a}, \mathbf{c}) \mapsto \mathbf{z}$.
...
$h:\mathbb{R}^2 \times \mathbb{R} \times \mathbb{R} \to \mathbb{R}$
given by
...
with stage input types $a \in \mathbb{R}^2$,
$b \in \mathbb{R}$, and $c \in \mathbb{R}$.
...
- a sequence of elements (scalars or vectors)
  $\mathbf{x} = (x_1, \dots, x_N)$, with
  $x_i \in \mathbb{R}^n$
- a binary operation
  $\otimes: \mathbb{R}^m\times \mathbb{R}^n\to \mathbb{R}^m$
...
Let us look at an example. Suppose
$\otimes: \mathbb{R} \times \mathbb{R}^3 \to \mathbb{R}$
defined as 
...
```math
\mathbf{reduce}_{\otimes, N}: (a_0, \mathbf{x})
\mapsto a_0 \otimes x_1 \ldots \otimes x_n.
```
...
Consider an input argument consisting of a sequence
$\mathbf{x} = (x_1, \ldots, x_n)$ with
$x_i\in\mathbb{R}^2$, and a function
$u:\mathbb{R}^2\to\mathbb{R}$ defined by
$u(x_i) = \sin(\Vert x_i \Vert^2)$.

The map operation applies $u$ to each entry of the sequence and produces
$y = (u(x_1), \ldots, u(x_n))$.
That output sequence is then fed into a reduce-with-addition step, which
computes the sum $s = a_0 + y_1 + \cdots + y_n$.
Lastly, the result of the reduce operation is fed into the post-processing
function $h(s, b) = bs^3$, which produces the final result, $z$, as shown
...
Firstly, let us introduce function $u$ and the corresponding map operator
...
Suppose we have a symbol $x\in\mathbb{R}^n$ and a function
$G({}\cdot{}, p):\mathbb{R}^n\to \mathbb{R}^n$, where $p$ is a parameter.
For an integer $N$ and a sequence of parameter symbols,
$p_0, \ldots, p_{N-1}$ we define the following sequence:
```math
\begin{align}
x_0 ={}& x, \\
x_{k+1} ={}& G(x_k, p_k),
...
```math
\mathbf{repeat}_{G, N}:x \mapsto x_N
```
...
```math
\mathbf{repeat}_{G, 2}(x, p_0, p_1) = G(G(x, p_0), p_1).
```
...
```math
G(x, p) = \begin{bmatrix}
p_1 x_1 + p_2  \sin(x_1x_2) \\
\tfrac{1}{2}x_1 + p_3 x_2
\end{bmatrix}.
```
...
More formally, suppose we have a sequence of functions
$F_i:\mathbb{R}^n\times \mathbb{R}^{m_i} \to \mathbb{R}^n$
...
```math
\begin{align}x_0 ={}& x \\
x_{i+1} ={}& F_i(x_i, p_i), i=0, \ldots, K-1
\end{align}
```
We define the *chain* function

```math
\mathbf{chain}_{((F_0, p_0), \ldots, (F_K, p_K))}(x, p_0, \ldots, p_K) = x_K.
```

Let us have a look at an example. Suppose $x\in\mathbb{R}^2$,
$p_1\in\mathbb{R}$, and $p_1, p_3\in\mathbb{R}^2$, while $p_2 = p_1$. 
Let 
...
```math
\mathbf{chain}_{((F, p_0), (G, p_1), (G, p_2), (H, p_3))}(x, p_0, p_1, p_3),
```
as follows
