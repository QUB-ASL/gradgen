...
$\mathrm\{map\}_\{u, N\}: \mathbf\{x\} \mapsto \mathbf\{y\}$.
...
In other words, it is the mapping
$\mathrm\{zip\}_\{b, M\}: (\mathbf\{a\}, \mathbf\{c\}) \mapsto \mathbf\{z\}$.
...
$$\mathrm\{reduce\}_\{\otimes, N\}: (a_0, \mathbf\{x\})
...
$$\mathrm\{repeat\}_\{G, N\}:x \mapsto x_\{N\},$$

where $x_N$ is produced by the above sequence. For example, for $N=2$,
we have 

$$\mathrm\{repeat\}_\{G, 2\}(x, p_0, p_1) = G(G(x, p_0), p_1).$$
...
$$\mathrm\{chain\}_\{(F_0, p_0)\ldots(F_\{K-1\}, p_\{K-1\})\}
(x, p_0, \ldots, p_\{K-1\}) = x_K$$

Let us have a look at an example. Suppose $x\in\mathbb\{R\}^2$,
$p_0\in\mathbb\{R\}$, and $p_1, p_3\in\mathbb\{R\}^2$. Let 
$$\begin\{align\}
F(x, p_0) ={}& p_0x,
\\\\
