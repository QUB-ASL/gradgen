---
sidebar_position: 5
slug: /basics/rust_py_iface
description: "Expose generated Rust crates to Python with Gradgen's optional Python bridge, workspace helpers, and metadata inspection APIs."
tags:
  - basics
  - rust
  - python-interface
...
`cargo build --release` for the generated low-level crate by default and
raises an informative error if `cargo` is not installed. If you want a faster
debug-style build during development, use
`RustBackendConfig().with_build_profile("debug")`.
