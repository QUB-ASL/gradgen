...
fn main() {
    let n_pred_max = 500;
    let n_runs= 5000;
    let mut ws = quadcopter_test::BackwardGradientWorkspace::new(n_pred_max);
    let nu =  quadcopter_test:: num_inputs();
    let nx =  quadcopter_test::num_states();
    let x0 = vec![1.0; nx];
    ...
    for n_pred in (10..=n_pred_max).step_by(5){
        ...
        for _j in 1..n_runs {
            let now = Instant::now();
            quadcopter_test::total_cost_gradient_bw(&x0, &u_seq, &mut grad, &mut ws, n_pred);
